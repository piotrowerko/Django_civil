    ...
    def compute_reinf_T(self):
        ...
        if m_rd < self.m_sd:  # the compression zone extends below the plate
            ...
            try:
                ...
                remark = 'WARNING: complex roots of equation system - try diff. input'
                return (0, 0, 0, 0, remark), remark
            if ksi_eff <= ksi_eff_lim:
                print(f'only the As1 reinforcement is sufficient {ksi_eff}, {ksi_eff_lim}')
                remark = 'only the As1 reinforcement is sufficient'
                a_s12 = ksi_eff * d * self.b * f_cd / f_yd
                a_s2 = 0
                a_s1 = a_s11 + a_s12
            else:
                print('As2 reinforcement is necessary')
                remark = 'As2 reinforcement is necessary'
            ...
            remark = 'The compression zone does NOT extends below the plate'
            return self.compute_reinf_rect(b=self.beff), remark
def main():
    my_rc_cross_sec = TCrReinf(name='moj_przekr_T',
                                b=0.5, # [m]
                                h=1.2, # [m]
                                hsl=0.2, #[m] thickness of upper slab
    ...
    print(f'upper needs: {res[0][3]} x fi{my_rc_cross_sec.fi} >= {res[0][2]}')
    print(res[1])
