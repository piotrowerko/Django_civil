...
from .serializers import RegistrationSerializer, AccountPropertiesSerializer, ChangePasswordSerializer # from account.api.serializers import RegistrationSerializer
...
def registration_view(request):
    ...
    if request.method == 'POST':
        data = {}
        email = request.data.get('email', '0').lower()  # It will get and value if exists or return '0' if key not exists
        if validate_email(email) != None:
            data['error_message'] = 'That email is already in use.'
            data['response'] = 'Error'
            return Response(data)

        username = request.data.get('username', '0')  # It will get and value if exists or return '0' if key not exists
